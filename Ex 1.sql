...
---12:
update dept SET LOCATION='Sousse'
where DeptNum=(select MIN(deptNUM) from DEPT);
---13:
update emp set commission=commission+(commission*10)/100
where salary>1500;
---14:
update emp set deptNUm=(select max(deptnum) from emp)
where salary<=1000;
